...
def spectrogram_paddle(y, n_fft, sampling_rate, hop_size, win_size, center=False):
    ...
    if wnsize_dtype not in hann_window:
        hann_window[wnsize_dtype] = paddle.audio.functional.get_window(window='hann', win_length=win_size, dtype=y.dtype)
    # TODO
    # y = paddle.nn.functional.pad(y.unsqueeze(1), (int((n_fft - hop_size) / 2), int((n_fft - hop_size) / 2)),
    #                              mode='reflect')
    # y = y.squeeze(1)
...
def mel_spectrogram_paddle(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False):
    ...
    if fmax_dtype_device not in mel_basis:
        mel = librosa_mel_fn(sr=sampling_rate, n_fft=n_fft, n_mels=num_mels, fmin=fmin, fmax=fmax)
        mel_basis[fmax_dtype_device] = paddle.to_tensor(mel, dtype=y.dtype)
    if wnsize_dtype_device not in hann_window:
        hann_window[wnsize_dtype_device] = paddle.to_tensor(win_size, dtype=y.dtype)
