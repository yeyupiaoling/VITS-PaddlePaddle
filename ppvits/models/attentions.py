...
class MultiHeadAttention(nn.Layer):
    ...
    def attention(self, query, key, value, mask=None):
        ...
        scores = paddle.matmul(query / math.sqrt(self.k_channels), key.transpose([0, 1, 3, 2]))
    ...
    def _attention_bias_proximal(self, length):
        ...
        """
        r = paddle.arange(length, dtype=paddle.float32)
...
class FFN(nn.Layer):
    ...
    def forward(self, x, x_mask):
        ...
        if self.activation == "gelu":
            x = x * paddle.nn.functional.sigmoid(1.702 * x)
        else:
